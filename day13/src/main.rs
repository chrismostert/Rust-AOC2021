...
fn make_grid(input: &str) -> Grid {
    input.lines().fold(HashMap::default(), |mut acc, x| {
        ...
        acc.insert((x.parse().unwrap(), y.parse().unwrap()), true);
        acc
    })
}
...
fn get_instructions(input: &'static str) -> impl Iterator<Item=FoldInstruction> {
    input.lines().map(|line| {
        let (t, v) = line.split_whitespace().nth(2).unwrap().split_once('=').unwrap();
        match t {
    ...
    match instruction {
            ...
            for x in 0..w {
                for y in line+1..h {
                    if grid.get(&(x,y)).is_some() {
                        grid.insert((x, line-(y-line)), true);
                        grid.remove(&(x,y));
                    }
                }
            }
        ...
        FoldInstruction::Left(line) => {
            for x in line+1..w {
                for y in 0..h {
                    if grid.get(&(x,y)).is_some() {
                        grid.insert((line-(x-line), y), true);
                        grid.remove(&(x,y));
                    }
                }
            }
...
fn main() {
    ...
    for (i, instruction) in instructions.enumerate() {
        ...
        if i == 0 {
            println!("Part 1: {}", grid.keys().count());
        }  
    }
    println!("Part 2:");
    print_grid(&grid);
